...
let cekUser = (satu, dua) => { 
    let x1 = false
    Object.keys(xeonverifieduser).forEach((i) => {
      if (xeonverifieduser[i].id == dua){x1 = i}})
      if (x1 !== false) {
        if (satu == "id"){ return xeonverifieduser[x1].id }
        if (satu == "name"){ return xeonverifieduser[x1].name }
        if (satu == "seri"){ return xeonverifieduser[x1].seri }
        if (satu == "premium"){ return xeonverifieduser[x1].premium }
        if (satu == "block"){ return xeonverifieduser[x1].block }
      }
...
const makeid = (length) => {
    ...
    function mentions(teks, mems = [], id) {
        ...
        let res = Zex.sendMessage(from, { text: teks, mentions: mems })
        return res
        } else {
        let res = Zex.sendMessage(from, { text: teks, mentions: mems }, { quoted: m })
        return res
        }
        }
        const XeonTheCreator = ['${setting.ownerNumber}',"6285381568989@s.whatsapp.net"].includes(sender) ? true : false
...
///////////////////////////////////////////////// PROGRAM MULAI ANJG ///////////////////////////////////////////////////////////////////////
 ...
 if (cekUser("id", sender) == null) {
    ...
    var res_us = `${makeid(10)}`
     ...
     let object_user = {"id": sender, "name": user_name, "seri": res_us, "premium": false, "block":false }
     xeonverifieduser.push(object_user)
     fs.writeFileSync('./database/user.json', JSON.stringify(xeonverifieduser, 2, null))
...
switch (command) {
            ...
            case 'deluser':
  {
    if (isOwner)
    {
      ...
      if (number_one + '@s.whatsapp.net' == `${ownerNumber}@s.whatsapp.net`) return reply("Tidak bisa menghapus owner")
      let index = xeonverifieduser.findIndex(user => user.id === number_one + '@s.whatsapp.net')
      if (index !== -1) {
        xeonverifieduser.splice(index, 1)
        fs.writeFileSync('./database/user.json', JSON.stringify(xeonverifieduser, null, 2))
        reply('Data pengguna berhasil dihapus dari database.')
      } else {
        reply('User tidak ditemukan di database.')
      }
    }
  // else {
  //   //   reply('Anda tidak memiliki izin untuk menghapus pengguna.')
  //   // }
...
if (cekUser("block", sender) == true) return reply(mess.blocked)
  ...
  //       var number = mentionUser[0]
  //       let index = xeonverifieduser.findIndex(user => user.id === number)
  //       if (index !== -1) {
  //           xeonverifieduser.splice(index, 1)
  //           fs.writeFileSync('./database/pengguna.json', JSON.stringify(xeonverifieduser, null, 2))
  //           reply('Data pengguna berhasil dihapus dari database.')
  //       } else {
  //         reply('User Belum Terdaftar Di Database')
  //       }
  //     } else {
  //       reply('Tag orang yang mau dihapus dari grup\n\n*Contoh:* .deluser @tag')
  //     }break;
  //   }
  ...
  // }
    ...
    // if (!q) return reply('*Contoh:*\n.delprem 628xxx')
    let number_one = sender
    if (number_one == '6285381568989@s.whatsapp.net') return reply("Gabisa delete owner cuy")
    let index = xeonverifieduser.findIndex(user => user.id === number_one)
    if (index !== -1) {
      xeonverifieduser.splice(index, 1)
      fs.writeFileSync('./database/user.json', JSON.stringify(xeonverifieduser, null, 2))
      reply('Data pengguna berhasil dihapus dari database.')
...
"containsAutoReply": true,
"mediaType": 1, 
"thumbnail": fs.readFileSync("./media/theme/1.jpg"),
"mediaUrl": `${wagc}`,
"sourceUrl": `${wagc}`
}
}
